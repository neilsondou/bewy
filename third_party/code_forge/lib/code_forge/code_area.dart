...
class CodeForge extends StatefulWidget {
  ...
  /// Whether built-in editor keyboard shortcuts are enabled.
  ///
  /// Disable this when the host application provides its own shortcut system.
  final bool enableEditorShortcuts;
  ...
  const CodeForge({
    ...
    this.enableEditorShortcuts = true,
...
class _CodeForgeState extends State<CodeForge> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _hoverDetailsStyle =
        ...
        HoverDetailsStyle(
          shape: RoundedRectangleBorder(
            ...
            side: BorderSide(
              color: _editorTheme['root']!.color?.withAlpha(50) ?? Colors.grey[400]!,
              width: 0.5,
            ),
          ),
          backgroundColor:
          ...
          textStyle:
              (() {
                final color = _editorTheme['root']!.color;
                final fontSize = (widget.textStyle?.fontSize ?? _editorTheme['root']!.fontSize) ?? 13.0;
                return TextStyle(
                  color: color,
                  fontSize: fontSize,
                );
    ...
    _diagnosticsNotifier.addListener(_onDiagnosticsChanged);
  ...
  String _getSuggestionCacheKey(dynamic item) {
    if (item is LspCompletion) {
      ...
      final importUri =
          (item.importUri != null && item.importUri!.isNotEmpty)
              ? item.importUri![0]
              : '';
  ...
  Future<void> _fetchCodeActionsForCurrentPosition() async {
    ...
    final actions = await _controller.lspConfig!.getCodeActions(
      ...
      diagnostics:
          _diagnosticsNotifier.value.map((item) => item.toJson()).toList(),
    );
  ...
  Future<void> _fetchHoverContent(Map<String, int> lineChar) async {
    ...
    try {
      ...
      // Show diagnostic immediately if available, before waiting for LSP hover
      if (diagnosticMessage.isNotEmpty && _controller.lspConfig != null) {
        final earlyResult = {
          'diagnostic': diagnosticMessage,
          'severity': severity,
          'hover': '',
          'startLine': diagnostic.range['start']?['line'] ?? 0,
          'startCharacter': diagnostic.range['start']?['character'] ?? 0,
        };
        if (_hoverNotifier.value != null &&
            _hoverNotifier.value!.$2['line'] == line &&
            _hoverNotifier.value!.$2['character'] == character) {
          _hoverContentNotifier.value = earlyResult;
        }
      } else {
        _hoverContentNotifier.value = null;
      }
      ...
      final result = {
        ...
        'severity': severity,
        'hover': hoverMessage,
        'startLine': diagnostic.message.isNotEmpty
            ? (diagnostic.range['start']?['line'] ?? 0)
            : 0,
        'startCharacter': diagnostic.message.isNotEmpty
  ...
  void dispose() {
    ...
    _diagnosticsNotifier.removeListener(_onDiagnosticsChanged);
              ...
              child: Stack(
                children: [
                    ...
                    builder: (_, offset, __) {
                      return ValueListenableBuilder(
                        ...
                        builder: (_, signature, __) {
                          ...
                          final desiredWidth =
                              screenWidth < 700 ? screenWidth * 0.63 : 420.0;
                  ...
                  ValueListenableBuilder(
                    ...
                    builder: (_, hov, c) {
                      if (hov == null || !widget.enableSuggestions) {
                        return SizedBox.shrink();
                      }
                      final Offset position = hov.$1;
                      final width =
                          _isMobile ? screenWidth * 0.63 : screenWidth * 0.3;
                      final maxHeight = _isMobile ? screenHeight * 0.4 : 420.0;
                        ...
                        child: MouseRegion(
                          ...
                          child: ValueListenableBuilder<Map<String, dynamic>?>(
                            ...
                            builder: (_, data, __) {
                              ...
                              final startLine = data['startLine'] ?? 0;
                              final startCharacter = data['startCharacter'] ?? 0;
                              ...
                              final rootColor = _editorTheme['root']!.color ?? Colors.white;
                              final rootBg = _editorTheme['root']!.backgroundColor ?? const Color(0xFF1E1E1E);
                              final borderColor = rootColor.withAlpha(20);
                              final subtleText = rootColor.withAlpha(140);
                              ...
                              return ConstrainedBox(
                                ...
                                child: Container(
                                  ...
                                  children: [
                                            ...
                                            Expanded(
                                              child: Column(
                                                ...
                                                children: [
                                                  Text(
                                                    diagnosticMessage,
                                                    style: _hoverDetailsStyle.textStyle.copyWith(
                                                      color: rootColor.withAlpha(220),
                                                      height: 1.3,
                                                    ),
                                                  ),
                                                  const SizedBox(height: 2),
                                                  Text(
                                                    '${severity == 1 ? 'Error' : severity == 2 ? 'Warning' : 'Info'} [Ln ${(startLine as int) + 1}, Col ${(startCharacter as int) + 1}]',
                                                    style: _hoverDetailsStyle.textStyle.copyWith(
                                                      color: subtleText,
                                                      fontSize: (_hoverDetailsStyle.textStyle.fontSize ?? 13) - 2,
                                                    ),
                                                  ),
                                                ],
                                              ),
                                    ...
                                    if (diagnosticMessage.isNotEmpty && hoverMessage.isNotEmpty)
                                      Container(
                                      ...
                                      Flexible(
                                        child: Padding(
                                          ...
                                          child: RawScrollbar(
                                              ...
                                              thickness: 3,
                                              thumbColor: rootColor.withAlpha(60),
                                              child: SingleChildScrollView(
                                                controller:
                                                ...
                                                child: MarkdownBlock(
                                                  ...
                                                  config: MarkdownConfig.darkConfig.copy(
                                                    configs: [
                                                      PConfig(
                                                        textStyle:
                                                            _hoverDetailsStyle.textStyle.copyWith(
                                                              color: rootColor.withAlpha(200),
                                                              height: 1.35,
                                                            ),
                                                      ),
                                                      PreConfig(
                                                        ...
                                                        styleNotMatched: TextStyle(
                                                          color: rootColor.withAlpha(180),
                                                        ),
                                                        padding: const EdgeInsets.all(6),
                                                        margin: const EdgeInsets.symmetric(vertical: 3),
                                                        decoration: BoxDecoration(
                                                          color: rootBg.withAlpha(200),
                                                          borderRadius:
                                                              BorderRadius.circular(3),
                                                          border: Border.all(
                                                            width: 1,
...
class _CodeFieldRenderer extends RenderBox implements MouseTrackerAnnotation {
  ...
  ui.Paragraph _buildParagraph(String text, {double? width}) {
    final builder =
        ui.ParagraphBuilder(_paragraphStyle)
  ...
  FoldRange? _getOrComputeFoldRange(int lineIndex) {
    ...
    final fold =
        (lspFoldRanges != null && lspFoldRanges.containsKey(lineIndex))
  ...
  _getCaretInfo() {
    ...
    final contentWidth =
        size.width - _gutterWidth - (innerPadding?.horizontal ?? 0);
    final paragraphWidth =
        lineWrap ? _wrapWidth : (isRTL ? max(contentWidth * 3, 10000.0) : null);
  ...
  void performLayout() {
    ...
    if (lineWrap) {
      ...
      if (hasActiveFolds) {
        ...
        final avgHeight =
            cachedCount > 0 ? cachedHeight / cachedCount : _lineHeight;
      ...
      if (_longLineWidth == 0 && lineCount > 0) {
        final viewTop =
            vscrollController.hasClients ? vscrollController.offset : 0.0;
        final viewBottom =
            viewTop +
    ...
    if (isMobile) {
      ...
      if (selection.isCollapsed) {
        ...
        if (_draggingStartHandle ||
          ...
          final selection = controller.selection;
          final dragOffset =
              _draggingStartHandle ? selection.start : selection.end;
          ...
          if (_cachedSelectionMagnifierParagraphs != null &&
            ...
            for (int line = startLine; line <= endLine; line++) {
              ...
              if (line == dragLine) {
                ...
                displayText =
                    lineText.length > 30 ? lineText.substring(0, 30) : lineText;
              }
          ...
          final fingerOffsetX =
              _draggingStartHandle ? 30.0 : (_draggingEndHandle ? -30.0 : 0.0);
        ...
        (() {
          if (gutterStyle.lineNumberStyle != null) {
            ...
            if (gutterTextStyle == null) {
              ...
              return gutterTextStyle.copyWith(
  ...
  void _drawIndentGuides(
    ...
    void processLine(int i) {
      ...
      final indentLevel =
          _lineIndentCache.containsKey(i)
              ? _lineIndentCache[i]!
  ...
  void _drawBracketBox(
    ...
    final contentWidth =
        size.width - _gutterWidth - (innerPadding?.horizontal ?? 0);
    final paragraphWidth =
        lineWrap ? _wrapWidth : (isRTL ? contentWidth : null);
  ...
  void _drawDiagnostics(
    ...
    for (final diagnostic in sortedDiagnostics) {
      ...
      for (int lineIndex = startLine; lineIndex <= endLine; lineIndex++) {
        ...
        final boxes =
            _diagnosticPathCache.containsKey(boxKey)
    ...
    for (final highlight in highlights) {
      ...
      for (int lineIndex = startLine; lineIndex <= endLine; lineIndex++) {
        ...
        if (lineText.isNotEmpty) {
          ...
          final boxes =
              _searchHighlightCache.containsKey(boxKey)
  ...
  void _drawFoldedLineHighlights(
    ...
    for (final foldRange in controller.foldings.values.where(
      ...
      final lineHeight =
          lineWrap ? _getWrappedLineHeight(foldStartLine) : _lineHeight;
  ...
  void _drawSelection(
    ...
    for (int lineIndex = startLine; lineIndex <= endLine; lineIndex++) {
      ...
      if (lineSelStart < lineSelEnd && lineText.isNotEmpty) {
        ...
        final screenX =
            isRTL ? offset.dx + textX + contentWidth - 8 : offset.dx + textX;
        final screenY =
            offset.dy +
            (innerPadding?.top ?? 0) +
            lineY -
  ...
  void _drawAiGhostText(
    ...
    final defaultGhostColor = (textStyle?.color ??
  ...
  void _drawLineHighlight(
    ...
    final lineHeight =
        lineWrap ? _getWrappedLineHeight(highlightLine) : _lineHeight;
    ...
    final highlightColor = (textStyle?.color ??
            editorTheme['root']?.color ??
            Colors.yellow)
  ...
  void _drawInlayHints(
    ...
    for (final entry in hintsByLine.entries) {
      ...
      for (int i = 0; i < lineHints.length; i++) {
        ...
        final style =
            hint.kind == InlayHintKind.type ? typeHintStyle : paramHintStyle;
        final hintText = hint.paddingLeft ? ' ${hint.text}' : hint.text;
        final displayText = hint.paddingRight ? '$hintText ' : hintText;
    ...
    if (event is PointerHoverEvent) {
      ...
      if ((hoverNotifier.value == null || !isHoveringPopup.value) &&
          (_isOffsetOverWord(textOffset) ||
              _isOffsetOverDiagnostic(textOffset))) {
        _hoverTimer?.cancel();
  ...
  bool _isOffsetOverDiagnostic(int offset) {
    ...
    for (final diag in diagnostics) {
      final diagStartLine = diag.range['start']?['line'] as int? ?? -1;
      final diagEndLine = diag.range['end']?['line'] as int? ?? -1;
      final diagStartChar = diag.range['start']?['character'] as int? ?? 0;
      final diagEndChar = diag.range['end']?['character'] as int? ?? 0;
      if (line < diagStartLine || line > diagEndLine) continue;
      if (line == diagStartLine && line == diagEndLine) {
        if (character >= diagStartChar && character < diagEndChar) return true;
      } else if (line == diagStartLine) {
        if (character >= diagStartChar) return true;
      } else if (line == diagEndLine) {
        if (character < diagEndChar) return true;
      } else {
        return true;
      }
    }
