...
class CodeForgeController implements DeltaTextInputClient {
          ...
          if (data['method'] == 'textDocument/publishDiagnostics') {
            ...
            if (rawDiagnostics.isNotEmpty) {
              ...
              for (final item in rawDiagnostics) {
                ...
                if (lspConfig!.disableError && lspConfig!.disableWarning) {
                  severity = 0;
                }
        ...
        _debounceTimer = Timer(const Duration(milliseconds: 200), () async {
          ...
          if (_isTyping && selection.extentOffset > 0) {
            ...
            if (prefix.isNotEmpty) {
              _suggestions =
                  _suggestions.where((s) => s.startsWith(prefix)).toList();
            }
  ...
  set openedFile(String? file) {
    ...
    if (lspConfig != null && _lspReady) {
      (() async {
        try {
          if (previousFile != null && previousFile != _openedFile) {
            await lspConfig!.closeDocument(previousFile);
          }
          if (_openedFile != null) {
            await lspConfig!.openDocument(_openedFile!);
            await lspConfig!.updateDocument(_openedFile!, text);
          }
        } catch (e) {
          debugPrint('Error opening document for LSP: $e');
        }
      })();
    }
  }
  ...
  void adjustLspFoldRangesForLineChange(int editLine, int lineDelta) {
    ...
    _lspFoldRanges =
        adjustedLspFoldRanges.isEmpty ? null : adjustedLspFoldRanges;
  ...
  void findWord(
    ...
    while (offset < textToSearch.length) {
      ...
      if (matchWholeWord) {
        ...
        final after =
            index + word.length < searchText.length
  ...
  void _handleReplacement(
    ...
    final deletedText =
        range.start < range.end ? _rope.substring(range.start, range.end) : '';
